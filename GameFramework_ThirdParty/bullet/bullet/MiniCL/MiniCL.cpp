                       ...
                       cl_event *       /* event */) CL_API_SUFFIX__VERSION_1_0
{
	...
	for (unsigned int ii=0;ii<work_dim;ii++)
	{
		...
		int numWorkItems = (int)global_work_size[ii];
	...
	} else
	{
		...
		else
		{
			...
			{	// this is only for __local memory qualifier
				void* ptr = localBufMalloc((int)arg_size);
			...
			kernel->m_argSizes[arg_index] = (int)arg_size;
               ...
               cl_int *         errcode_ret ) CL_API_SUFFIX__VERSION_1_0
{
	...
	int nameLen = (int)strlen(kernel_name);
